  ...
  <li class="commit-item w-full border-b border-gray-700 pb-4 mb-2">
    <div class="flex flex-row items-center justify-between gap-4 w-full">
      <div class="flex flex-col min-w-0 w-full">
        <div
          class="commit-message font-medium truncate overflow-ellipsis whitespace-nowrap max-w-2xl md:max-w-3xl lg:max-w-5xl"
        >
          {{ props.commit.commit.message }}
        </div>
        <div class="commit-meta text-xs text-gray-400 flex flex-col gap-0 mt-1">
          <span class="commit-author">{{
            props.commit.author?.login || props.commit.commit.author.name
          }}</span>
      ...
      <div class="flex flex-row gap-2">
        ...
        >
          <component
            :is="props.commit.favorited ? HeartSolid : HeartOutline"
            class="w-4 h-4"
            :class="props.commit.favorited ? 'text-pink-400' : 'text-gray-400'"
          />
        </button>
        <button
          @click="$emit('select')"
