      ...
      >
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2 gap-2">
          ...
          >
            ...
            <component
              :is="store.selectedRepo === repo.name ? ChevronUpIcon : ChevronDownIcon"
              class="w-4 h-4"
            />
          </button>
        </div>
        <p class="text-gray-300 text-sm w-full break-words max-w-2xl sm:max-w-3xl">
        ...
        <div v-if="store.selectedRepo === repo.name" class="mt-4">
          ...
          <div v-if="store.loading" class="text-gray-400 text-xs mt-2 flex items-center gap-2">
            <ArrowPathIcon class="animate-spin h-4 w-4 text-primary-400" />
            Loading...
          </div>
        </div>
      </li>
